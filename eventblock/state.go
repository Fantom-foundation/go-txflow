...
var (
	ErrInvalidEventBlockSignature = errors.New("Error invalid event block signature")
	ErrAddingEventBlock           = errors.New("Error adding event block")
	ErrEventBlockHeightMismatch   = errors.New("Error event block height mismatch")
)
...
// EventBlockState handles execution of the event block creation.
// It processes transactions into event blocks
type EventBlockState struct {
	cmn.BaseService
	...
	// store event blocks and commits
	blockStore sm.BlockStore

	// create and execute event blocks
...
type StateOption func(*EventBlockState)

// NewEventBlockState returns a new ConsensusState.
func NewEventBlockState(
	config *cfg.ConsensusConfig,
	state sm.State,
	blockExec *sm.BlockExecutor,
	blockStore sm.BlockStore,
	...
	ebs.decideProposal = ebs.defaultDecideProposal
	ebs.doPrevote = ebs.defaultDoPrevote
	ebs.setProposal = ebs.defaultSetProposal

	ebs.updateToState(state)

	// Don't call scheduleRound0 yet.
	// We do that upon Start().
	ebs.reconstructLastCommit(state)
	ebs.BaseService = *cmn.NewBaseService(nil, "EventBlockState", ebs)
...
func (ebs *EventBlockState) SetLogger(l log.Logger) {
	ebs.BaseService.Logger = l
	ebs.timeoutTicker.SetLogger(l)
}

// SetEventBus sets event bus.
func (ebs *EventBlockState) SetEventBus(b *types.EventBus) {
	ebs.eventBus = b
	ebs.blockExec.SetEventBus(b)
}

// StateMetrics sets the metrics.
func StateMetrics(metrics *Metrics) StateOption {
	return func(ebs *EventBlockState) { ebs.metrics = metrics }
}

// String returns a string.
func (ebs *EventBlockState) String() string {
	// better not to access shared variables
	return fmt.Sprintf("EventBlockState") //(H:%v R:%v S:%v", cs.Height, cs.Round, cs.Step)
}

// GetState returns a copy of the chain state.
func (ebs *EventBlockState) GetState() sm.State {
	ebs.mtx.RLock()
	defer ebs.mtx.RUnlock()
	return ebs.state.Copy()
}

// GetLastHeight returns the last height committed.
// If there were no blocks, returns 0.
func (ebs *EventBlockState) GetLastHeight() int64 {
	ebs.mtx.RLock()
	defer ebs.mtx.RUnlock()
	return ebs.RoundState.Height - 1
}

// GetRoundState returns a shallow copy of the internal consensus state.
func (ebs *EventBlockState) GetRoundState() *cstypes.RoundState {
	ebs.mtx.RLock()
	rs := ebs.RoundState // copy
	ebs.mtx.RUnlock()
	return &rs
}

// GetRoundStateJSON returns a json of RoundState, marshalled using go-amino.
func (ebs *EventBlockState) GetRoundStateJSON() ([]byte, error) {
	ebs.mtx.RLock()
	defer ebs.mtx.RUnlock()
	return cdc.MarshalJSON(ebs.RoundState)
}

// GetRoundStateSimpleJSON returns a json of RoundStateSimple, marshalled using go-amino.
func (ebs *EventBlockState) GetRoundStateSimpleJSON() ([]byte, error) {
	ebs.mtx.RLock()
	defer ebs.mtx.RUnlock()
	return cdc.MarshalJSON(ebs.RoundState.RoundStateSimple())
}

// GetValidators returns a copy of the current validators.
func (ebs *EventBlockState) GetValidators() (int64, []*types.Validator) {
	ebs.mtx.RLock()
	defer ebs.mtx.RUnlock()
	return ebs.state.LastBlockHeight, ebs.state.Validators.Copy().Validators
}

// SetPrivValidator sets the private validator account for signing votes.
func (ebs *EventBlockState) SetPrivValidator(priv types.PrivValidator) {
	ebs.mtx.Lock()
	ebs.privValidator = priv
	ebs.mtx.Unlock()
}

// SetTimeoutTicker sets the local timer. It may be useful to overwrite for testing.
func (ebs *EventBlockState) SetTimeoutTicker(timeoutTicker TimeoutTicker) {
	ebs.mtx.Lock()
	ebs.timeoutTicker = timeoutTicker
	ebs.mtx.Unlock()
}

// LoadCommit loads the commit for a given height.
func (ebs *EventBlockState) LoadCommit(height int64) *types.Commit {
	ebs.mtx.RLock()
	defer ebs.mtx.RUnlock()
	if height == ebs.blockStore.Height() {
		return ebs.blockStore.LoadSeenCommit(height)
	}
	return ebs.blockStore.LoadBlockCommit(height)
}

// OnStart implements cmn.Service.
// It loads the latest state via the WAL, and starts the timeout and receive routines.
func (ebs *EventBlockState) OnStart() error {
	if err := ebs.evsw.Start(); err != nil {
		return err
	}

	// we may set the WAL in testing before calling Start,
	// so only OpenWAL if its still the nilWAL
	if _, ok := ebs.wal.(nilWAL); ok {
		walFile := ebs.config.WalFile()
		wal, err := ebs.OpenWAL(walFile)
		if err != nil {
			ebs.Logger.Error("Error loading EventBlockState wal", "err", err.Error())
			return err
		}
		ebs.wal = wal
	}
	...
	if err := ebs.timeoutTicker.Start(); err != nil {
		return err
	}
	...
	if ebs.doWALCatchup {
		if err := ebs.catchupReplay(ebs.Height); err != nil {
			// don't try to recover from data corruption error
			if IsDataCorruptionError(err) {
				ebs.Logger.Error("Encountered corrupt WAL file", "err", err.Error())
				ebs.Logger.Error("Please repair the WAL file before restarting")
...
----`)
			...
			ebs.Logger.Error("Error on catchup replay. Proceeding to start ConsensusState anyway", "err", err.Error())
	...
	go ebs.receiveRoutine(0)

	// schedule the first round!
	// use GetRoundState so we don't race the receiveRoutine for access
	ebs.scheduleRound0(ebs.GetRoundState())
...
func (ebs *EventBlockState) startRoutines(maxSteps int) {
	err := ebs.timeoutTicker.Start()
	if err != nil {
		ebs.Logger.Error("Error starting timeout ticker", "err", err)
		return
	}
	go ebs.receiveRoutine(maxSteps)
}

// OnStop implements cmn.Service.
func (ebs *EventBlockState) OnStop() {
	ebs.evsw.Stop()
	ebs.timeoutTicker.Stop()
...
func (ebs *EventBlockState) Wait() {
	<-ebs.done
}

// OpenWAL opens a file to log all consensus messages and timeouts for deterministic accountability
func (ebs *EventBlockState) OpenWAL(walFile string) (WAL, error) {
	wal, err := NewWAL(walFile)
	if err != nil {
		ebs.Logger.Error("Failed to open WAL for event block state", "wal", walFile, "err", err)
		return nil, err
	}
	wal.SetLogger(ebs.Logger.With("wal", walFile))
...
func (ebs *EventBlockState) updateHeight(height int64) {
	ebs.metrics.Height.Set(float64(height))
	ebs.Height = height
}

// enterNewRound(height, 0) at cs.StartTime.
func (ebs *EventBlockState) scheduleRound0(rs *cstypes.RoundState) {
	//cs.Logger.Info("scheduleRound0", "now", tmtime.Now(), "startTime", cs.StartTime)
	sleepDuration := rs.StartTime.Sub(tmtime.Now())
	ebs.scheduleTimeout(sleepDuration, rs.Height, 0, cstypes.RoundStepNewHeight)
}

// Attempt to schedule a timeout (by sending timeoutInfo on the tickChan)
func (ebs *EventBlockState) scheduleTimeout(duration time.Duration, height int64) {
	ebs.timeoutTicker.ScheduleTimeout(timeoutInfo{duration, height})
}

// Updates EventBlockState and increments height to match that of state.
func (ebs *EventBlockState) updateToState(state sm.State) {
	if ebs.CommitRound > -1 && 0 < ebs.Height && ebs.Height != state.LastBlockHeight {
		cmn.PanicSanity(fmt.Sprintf("updateToState() expected state height of %v but found %v",
			ebs.Height, state.LastBlockHeight))
	}
	if !ebs.state.IsEmpty() && ebs.state.LastBlockHeight+1 != ebs.Height {
		// This might happen when someone else is mutating cs.state.
		// Someone forgot to pass in state.Copy() somewhere?!
		cmn.PanicSanity(fmt.Sprintf("Inconsistent cs.state.LastBlockHeight+1 %v vs cs.Height %v",
			ebs.state.LastBlockHeight+1, ebs.Height))
	}
	...
	if !ebs.state.IsEmpty() && (state.LastBlockHeight <= ebs.state.LastBlockHeight) {
		ebs.Logger.Info("Ignoring updateToState()", "newHeight", state.LastBlockHeight+1, "oldHeight", ebs.state.LastBlockHeight+1)
		ebs.newStep()
		return
	}
	...
	// RoundState fields
	ebs.updateHeight(height)
	ebs.updateRoundStep(0, cstypes.RoundStepNewHeight)
	if ebs.CommitTime.IsZero() {
		// "Now" makes it easier to sync up dev nodes.
		// We add timeoutCommit to allow transactions
		// to be gathered for the first block.
		// And alternative solution that relies on clocks:
		//  cs.StartTime = state.LastBlockTime.Add(timeoutCommit)
		ebs.StartTime = ebs.config.Commit(tmtime.Now())
	} else {
		ebs.StartTime = ebs.config.Commit(ebs.CommitTime)
	}

	ebs.Validators = validators
	ebs.LastValidators = state.LastValidators
	ebs.TriggeredTimeoutPrecommit = false
...
func (ebs *EventBlockState) newStep() {
	rs := ebs.RoundStateEvent()
	ebs.wal.Write(rs)
	ebs.nSteps++
	// newStep is called by updateToState in NewConsensusState before the eventBus is set!
	if ebscs.eventBus != nil {
		ebs.eventBus.PublishEventNewRoundStep(rs)
		ebs.evsw.FireEvent(types.EventNewRoundStep, &cs.RoundState)
	}
}
...
func (ebs *EventBlockState) receiveRoutine(maxSteps int) {
	onExit := func(ebs *EventBlockState) {
	...
	defer func() {
		if r := recover(); r != nil {
			evs.Logger.Error("EVENT BLOCK FAILURE!!!", "err", r, "stack", string(debug.Stack()))
			// stop gracefully
			//
	...
	for {
		if maxSteps > 0 {
			if ebs.nSteps >= maxSteps {
				ebs.Logger.Info("reached max steps. exiting receive routine")
				ebs.nSteps = 0
				return
			}
		}
		...
		select {
		case <-ebs.txNotifier.TxsAvailable():
			ebs.handleAvailable()
		case <-ebs.eventNotifier.EventsAvailable():
			ebs.handleAvailable()
		case ti := <-ebs.timeoutTicker.Chan(): // tockChan:
			ebs.wal.Write(ti)
...
func (ebs *EventBlockState) handleAvailable() {
	ebs.mtx.Lock()
	defer ebs.mtx.Unlock()
	// we only need to do this for round 0
	ebs.enterNewRound(ebs.Height, 0)
	ebs.enterPropose(ebs.Height, 0)
}
...
func (ebs *EventBlockState) isProposer(address []byte) bool {
	return bytes.Equal(ebs.Validators.GetProposer().Address, address)
}

func (ebs *EventBlockState) defaultDecideProposal(height int64) {
	var block *types.Block
	var blockParts *types.PartSet

	// Decide on block
	if ebs.ValidBlock != nil {
		// If there is valid block, choose that.
		block, blockParts = ebs.ValidBlock, ebs.ValidBlockParts
	} else {
		// Create a new proposal block from state/txs from the mempool.
		block, blockParts = ebs.createProposalBlock()
	...
	// Make proposal
	propBlockId := EventBlockID{Hash: block.Hash(), PartsHeader: blockParts.Header()}
...
func (ebs *EventBlockState) createProposalBlock() (block *types.Block, blockParts *types.PartSet) {
	proposerAddr := ebs.privValidator.GetPubKey().Address()
	return ebs.blockExec.CreateProposalBlock(ebs.Height, ebs.state, proposerAddr)
}
...
func (cs *ConsensusState) finalizeCommit(height int64) {
	...
	var err error
	stateCopy, err = cs.blockExec.ApplyBlock(stateCopy, EventBlockID{Hash: block.Hash(), PartsHeader: blockParts.Header()}, block)
	if err != nil {
...
func (cs *ConsensusState) signVote(type_ types.SignedMsgType, hash []byte, header types.PartSetHeader) (*types.Vote, error) {
	...
	vote := &types.Vote{
		...
		Type:             type_,
		BlockID:          EventBlockID{Hash: hash, PartsHeader: header},
	}
