...
func TestSignerRemoteRetryTCPOnly(t *testing.T) {
	...
	serviceEndpoint := privval.NewSignerServiceEndpoint(
		...
		cmn.RandStr(12),
		types.NewMockPV(),
		privval.DialTCPFn(ln.Addr().String(), testTimeoutReadWrite, ed25519.GenPrivKey()),
	)
	defer serviceEndpoint.Stop()

	privval.SignerServiceEndpointTimeoutReadWrite(time.Millisecond)(serviceEndpoint)
	privval.SignerServiceEndpointConnRetries(retries)(serviceEndpoint)

	assert.Equal(t, serviceEndpoint.Start(), privval.ErrDialRetryMax)
