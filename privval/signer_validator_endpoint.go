...
func (ve *SignerValidatorEndpoint) SignVote(chainID string, vote *ttypes.Vote) error {
	ve.mtx.Lock()
	defer ve.mtx.Unlock()
...
// SignVote implements PrivValidator.
func (ve *SignerValidatorEndpoint) SignTxVote(chainID string, vote *types.TxVote) error {
	ve.mtx.Lock()
	defer ve.mtx.Unlock()
	return ve.signer.SignTxVote(chainID, vote)
}

// SignProposal implements PrivValidator.
func (ve *SignerValidatorEndpoint) SignProposal(chainID string, proposal *ttypes.Proposal) error {
	ve.mtx.Lock()
	defer ve.mtx.Unlock()
		...
		for {
			...
			case <-ve.pingTicker.C:
				...
				if err != nil {
					...
					if err == privval.ErrUnexpectedResponse {
						return
					}
