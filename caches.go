...
type SigPool struct {
	items map[string]hashgraph.BlockSignature
}

func NewSigPool() *SigPool {
	return &SigPool{
		items: make(map[string]hashgraph.BlockSignature),
	}
}

func (sp *SigPool) Add(blockSignature hashgraph.BlockSignature) {
...
func (sp *SigPool) RemoveSlice(sigs []hashgraph.BlockSignature) {
	for _, s := range sigs {
...
func (sp *SigPool) Items() map[string]hashgraph.BlockSignature {
	return sp.items
}

func (sp *SigPool) Slice() []hashgraph.BlockSignature {
	res := []hashgraph.BlockSignature{}
