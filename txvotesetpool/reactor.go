...
func (txR *Reactor) Receive(chID byte, src p2p.Peer, msgBytes []byte) {
	...
	switch msg := msg.(type) {
	case *TxVoteSetMessage:
		peerID := txR.ids.GetForPeer(src)
		err := txR.txVoteSetPool.CheckTxWithInfo(msg.Tx, tmempool.TxInfo{SenderID: peerID})
		if err != nil {
			txR.Logger.Info("Could not check tx", "tx", TxVoteSetID(msg.Tx), "err", err)
		}
...
func (txR *Reactor) broadcastTxRoutine(peer p2p.Peer) {
	...
	for {
		...
		if next == nil {
			select {
			case <-txR.txVoteSetPool.TxsWaitChan(): // Wait until a tx is available
				if next = txR.txVoteSetPool.TxsFront(); next == nil {
					continue
				}
			case <-peer.Quit():
				return
		...
		txTx := next.Value.(*MempoolTxVoteSet)
		...
		if _, ok := txTx.Senders.Load(peerID); !ok {
			// send txTx
			msg := &TxVoteSetMessage{Tx: txTx.Tx}
...
func RegisterTxVoteSetPoolMessages(cdc *amino.Codec) {
	cdc.RegisterInterface((*TxpoolMessage)(nil), nil)
	cdc.RegisterConcrete(&TxVoteSetMessage{}, "tendermint/txvotesetpool/TxVoteSetMessage", nil)
}
...
type TxVoteSetMessage struct {
	Tx types.TxVoteSet
}

// String returns a string representation of the TxMessage.
func (m *TxVoteSetMessage) String() string {
	return fmt.Sprintf("[TxVoteMessage %v]", m.Tx)
}

type txVoteSetPoolIDs struct {
	mtx       sync.RWMutex
...
// peer.
func (ids *txVoteSetPoolIDs) ReserveForPeer(peer p2p.Peer) {
	ids.mtx.Lock()
...
func (ids *txVoteSetPoolIDs) nextPeerID() uint16 {
...
func (ids *txVoteSetPoolIDs) Reclaim(peer p2p.Peer) {
	ids.mtx.Lock()
	defer ids.mtx.Unlock()
...
func (ids *txVoteSetPoolIDs) GetForPeer(peer p2p.Peer) uint16 {
	ids.mtx.RLock()
...
func newTxVoteSetPoolIDs() *txVoteSetPoolIDs {
	return &txVoteSetPoolIDs{
