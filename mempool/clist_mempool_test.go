...
func checkTxs(t *testing.T, mempl mempool.Mempool, count int, peerID uint16) types.Txs {
	...
	for i := 0; i < count; i++ {
		...
		if err := mempl.CheckTxWithInfo(txBytes, nil, txInfo); err != nil {
			// Skip invalid txs.
...
func TestMempoolFilters(t *testing.T) {
	...
	tests := []struct {
		...
		{10, mempool.PreCheckAminoMaxBytes(10), nopPostFilter, 0},
		{10, mempool.PreCheckAminoMaxBytes(20), nopPostFilter, 0},
		{10, mempool.PreCheckAminoMaxBytes(22), nopPostFilter, 10},
		...
		{10, mempool.PreCheckAminoMaxBytes(10), mempool.PostCheckMaxGas(20), 0},
		{10, mempool.PreCheckAminoMaxBytes(30), mempool.PostCheckMaxGas(20), 10},
		{10, mempool.PreCheckAminoMaxBytes(22), mempool.PostCheckMaxGas(1), 10},
		{10, mempool.PreCheckAminoMaxBytes(22), mempool.PostCheckMaxGas(0), 0},
	}
	for tcIndex, tt := range tests {
		mempl.Update(1, emptyTxArr, abciResponses(len(emptyTxArr), abci.CodeTypeOK), tt.preFilter, tt.postFilter)
		checkTxs(t, mempl, tt.numTxsToCreate, UnknownPeerID)
		require.Equal(t, tt.expectedNumTxs, mempl.Size(), "mempool had the incorrect size, on test case %d", tcIndex)
		mempl.Flush()
	}
}
...
func TestMempoolRemoteAppConcurrency(t *testing.T) {
	...
	defer server.Stop()
	config := cfg.ResetTestRoot("mempool_test")
	mempl, cleanup := newMempoolWithAppAndConfig(cc, config)
	defer cleanup()
	...
	for i := 0; i < N; i++ {
		...
		mempl.CheckTxWithInfo(tx, nil, mempool.TxInfo{SenderID: uint16(peerID)})
	}
	err := mempl.FlushAppConn()
	require.NoError(t, err)
}
