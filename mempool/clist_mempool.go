...
func (mem *CListMempool) CheckTx(tx types.Tx, cb func(*abci.Response)) (err error) {
	return mem.CheckTxWithInfo(tx, cb, mempool.TxInfo{SenderID: mempool.UnknownPeerID})
}

func (mem *CListMempool) CheckTxWithInfo(tx types.Tx, cb func(*abci.Response), txInfo mempool.TxInfo) (err error) {
	mem.proxyMtx.Lock()
