...
// database keys
var (
	txFlowStateKey = []byte("txFlowStateKey")
)
...
type State struct {
	...
	// LastValidators is used to validate tx.LastCommit.
	// Validators are persisted to the database separately every time they change,
	// so we can query for historical validator sets.
	// Note that if s.LastTxHeight causes a valset change,
	// we set s.LastHeightValidatorsChanged = s.LastTxHeight + 1 + 1
...
// MakeVote builds a vote from the current state with the given txs
// Note it also takes a proposerAddress because the state does

func (state State) MakeVote(
	height int64,
	txs []types.Tx,
