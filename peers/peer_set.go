...
//NewPeerSetFromValidators creates a new PeerSet from a list of Peers
func NewPeerSetFromValidators(validators []*types.Validator) *PeerSet {
	peerSet := &PeerSet{
		ByPubKey: make(map[string]*Peer),
		ByID:     make(map[uint32]*Peer),
	}
	peers := make([]*Peer, len(validators))

	for _, validator := range validators {
		peer := NewPeer(validator.Address.String(), "", "")
		peerSet.ByPubKey[peer.PubKeyString()] = peer
		peerSet.ByID[peer.ID()] = peer
	}
