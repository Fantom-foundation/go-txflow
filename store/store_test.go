...
func makeTestCommit(height int64, timestamp time.Time) *ttypes.Commit {
	commitSigs := []*ttypes.CommitSig{{Height: height, Timestamp: timestamp}}
	return ttypes.NewCommit(ttypes.BlockID{}, commitSigs)
}
...
func makeBlock(height int64, state sm.State, lastCommit *ttypes.Commit) *types.Block {
	block, _ := state.MakeBlock(height, makeTxs(height), nil, lastCommit, nil, state.Validators.GetProposer().Address)
	return block
}
...
var (
	state       sm.State
	block       *ttypes.Block
	partSet     *ttypes.PartSet
	part1       *ttypes.Part
	part2       *ttypes.Part
	seenCommit1 *ttypes.Commit
)

func TestMain(m *testing.M) {
	var cleanup cleanupFunc
	state, _, cleanup = makeStateAndBlockStore(log.NewTMLogger(new(bytes.Buffer)))
	block := makeBlock(1, state, new(ttypes.Commit))
...
func TestBlockStoreSaveLoadBlock(t *testing.T) {
	...
	// save a block
	block := makeBlock(bs.Height()+1, state, new(ttypes.Commit))
	validPartSet := block.MakePartSet(2)
	seenCommit := makeTestCommit(10, tmtime.Now())
	bs.SaveBlock(block, partSet, seenCommit)
	require.Equal(t, bs.Height(), block.Header.Height, "expecting the new height to be changed")

	incompletePartSet := ttypes.NewPartSetFromHeader(ttypes.PartSetHeader{Total: 2})
	uncontiguousPartSet := ttypes.NewPartSetFromHeader(ttypes.PartSetHeader{Total: 0})
	uncontiguousPartSet.AddPart(part2)

	header1 := ttypes.Header{
		Height:  1,
		NumTxs:  100,
		ChainID: "block_test",
		Time:    tmtime.Now(),
	...
	tuples := []struct {
		block      *types.Block
		parts      *ttypes.PartSet
		seenCommit *ttypes.Commit
		wantErr    bool
		wantPanic  string
...

		{
			...
			wantPanic:         "unmarshal to ttypes.Commit failed",
		},

		{
			...
			parts:            validPartSet,
			seenCommit:       seenCommit1,
			wantPanic:        "unmarshal to ttypes.BlockMeta failed",
...
func TestLoadBlockPart(t *testing.T) {
	...
	require.Contains(t, panicErr.Error(), "unmarshal to ttypes.Part failed")
	...
	require.Equal(t, gotPart.(*ttypes.Part), part1,
...
func TestLoadBlockMeta(t *testing.T) {
	...
	res, _, panicErr = doFn(loadMeta)
	require.NotNil(t, panicErr, "expecting a non-nil panic")
	require.Contains(t, panicErr.Error(), "unmarshal to ttypes.BlockMeta")
...
func newBlock(hdr ttypes.Header, lastCommit *ttypes.Commit) *types.Block {
	return &types.Block{
		Header:     hdr,
