...
import (
	...
	cstypes "github.com/Fantom-foundation/go-txflow/consensus/types"
...
func NewValidatorStub(privValidator ttypes.PrivValidator, valIndex int) *validatorStub {
...
func (vs *validatorStub) signVote(voteType ttypes.SignedMsgType, hash []byte, header ttypes.PartSetHeader) (*ttypes.Vote, error) {
...
func signVote(vs *validatorStub, voteType ttypes.SignedMsgType, hash []byte, header ttypes.PartSetHeader) *ttypes.Vote {
...
func signVotes(voteType ttypes.SignedMsgType, hash []byte, header ttypes.PartSetHeader, vss ...*validatorStub) []*ttypes.Vote {
	votes := make([]*ttypes.Vote, len(vss))
	for i, vs := range vss {
...
func decideProposal(cs1 *ConsensusState, vs *validatorStub, height int64, round int) (proposal *ttypes.Proposal, block *types.Block) {
	cs1.mtx.Lock()
	...
	// Make proposal
	polRound, propBlockID := validRound, ttypes.BlockID{Hash: block.Hash(), PartsHeader: blockParts.Header()}
	proposal = ttypes.NewProposal(height, round, polRound, propBlockID)
...
func addVotes(to *ConsensusState, votes ...*ttypes.Vote) {
	for _, vote := range votes {
		to.peerMsgQueue <- msgInfo{Msg: &VoteMessage{vote}}
	}
}

func signAddVotes(to *ConsensusState, voteType ttypes.SignedMsgType, hash []byte, header ttypes.PartSetHeader, vss ...*validatorStub) {
...
func subscribeToVoter(cs *ConsensusState, addr []byte) <-chan tmpubsub.Message {
	votesSub, err := cs.eventBus.SubscribeUnbuffered(context.Background(), testSubscriber, ttypes.EventQueryVote)
	if err != nil {
		panic(fmt.Sprintf("failed to subscribe %s to %v", testSubscriber, ttypes.EventQueryVote))
	}
	ch := make(chan tmpubsub.Message)
	go func() {
		for msg := range votesSub.Out() {
			vote := msg.Data().(ttypes.EventDataVote)
...
func newConsensusState(state sm.State, pv ttypes.PrivValidator, app abci.Application) *ConsensusState {
	config := cfg.ResetTestRoot("consensus_state_test")
	return newConsensusStateWithConfig(config, state, pv, app)
}

func newConsensusStateWithConfig(thisConfig *cfg.Config, state sm.State, pv ttypes.PrivValidator, app abci.Application) *ConsensusState {
	blockDB := dbm.NewMemDB()
	return newConsensusStateWithConfigAndBlockStore(thisConfig, state, pv, app, blockDB)
}

func newConsensusStateWithConfigAndBlockStore(thisConfig *cfg.Config, state sm.State, pv ttypes.PrivValidator, app abci.Application, blockDB dbm.DB) *ConsensusState {
	// Get BlockStore
	...
	commitpool := mempl.NewCListMempool(thisConfig.Mempool, proxyAppConnMem, 0)
	commitpool.SetLogger(log.TestingLogger().With("module", "commitpool"))
	if thisConfig.Consensus.WaitForTxs() {
	...
	blockExec := sm.NewBlockExecutor(stateDB, log.TestingLogger(), proxyAppConnCon, mempool, commitpool, evpool)
...
func ensureNewEvent(ch <-chan tmpubsub.Message, height int64, round int, timeout time.Duration, errorMessage string) {
	...
	case <-time.After(timeout):
		panic(errorMessage)
	case msg := <-ch:
		roundStateEvent, ok := msg.Data().(ttypes.EventDataRoundState)
		if !ok {
...
func ensureNewRound(roundCh <-chan tmpubsub.Message, height int64, round int) {
	...
	case msg := <-roundCh:
		newRoundEvent, ok := msg.Data().(ttypes.EventDataNewRound)
		if !ok {
...
func ensureNewProposal(proposalCh <-chan tmpubsub.Message, height int64, round int) {
	...
	case msg := <-proposalCh:
		proposalEvent, ok := msg.Data().(ttypes.EventDataCompleteProposal)
		if !ok {
...
func ensureNewBlock(blockCh <-chan tmpubsub.Message, height int64) {
	...
	case msg := <-blockCh:
		blockEvent, ok := msg.Data().(ttypes.EventDataNewBlock)
		if !ok {
...
func ensureNewBlockHeader(blockCh <-chan tmpubsub.Message, height int64, blockHash cmn.HexBytes) {
	...
	case msg := <-blockCh:
		blockHeaderEvent, ok := msg.Data().(ttypes.EventDataNewBlockHeader)
		if !ok {
...
func ensureProposal(proposalCh <-chan tmpubsub.Message, height int64, round int, propID ttypes.BlockID) {
	select {
	case <-time.After(ensureTimeout):
		panic("Timeout expired while waiting for NewProposal event")
	case msg := <-proposalCh:
...
func ensurePrecommit(voteCh <-chan tmpubsub.Message, height int64, round int) {
	ensureVote(voteCh, height, round, ttypes.PrecommitType)
}

func ensurePrevote(voteCh <-chan tmpubsub.Message, height int64, round int) {
	ensureVote(voteCh, height, round, ttypes.PrevoteType)
}

func ensureVote(voteCh <-chan tmpubsub.Message, height int64, round int,
	voteType ttypes.SignedMsgType) {
	select {
	case <-time.After(ensureTimeout):
		panic("Timeout expired while waiting for NewVote event")
	case msg := <-voteCh:
		voteEvent, ok := msg.Data().(ttypes.EventDataVote)
		if !ok {
...
func randConsensusNet(nValidators int, testName string, tickerFunc func() TimeoutTicker,
	...
	for i := 0; i < nValidators; i++ {
		...
		app := appFunc()
		vals := ttypes.TM2PB.ValidatorUpdates(state.Validators)
...
func randConsensusNetWithPeers(nValidators, nPeers int, testName string, tickerFunc func() TimeoutTicker, appFunc func(string) abci.Application) ([]*ConsensusState, *ttypes.GenesisDoc, *cfg.Config, cleanupFunc) {
	...
	for i := 0; i < nPeers; i++ {
		...
		var privVal ttypes.PrivValidator
		if i < nValidators {
			privVal = privVals[i]
		} else {
...
func randGenesisDoc(numValidators int, randPower bool, minPower int64) (*ttypes.GenesisDoc, []ttypes.PrivValidator) {
	validators := make([]ttypes.GenesisValidator, numValidators)
	privValidators := make([]ttypes.PrivValidator, numValidators)
	for i := 0; i < numValidators; i++ {
		val, privVal := ttypes.RandValidator(randPower, minPower)
		validators[i] = ttypes.GenesisValidator{
			PubKey: val.PubKey,
			Power:  val.VotingPower,
		}
		privValidators[i] = privVal
	}
	sort.Sort(ttypes.PrivValidatorsByAddress(privValidators))
...
func randGenesisState(numValidators int, randPower bool, minPower int64) (sm.State, []ttypes.PrivValidator) {
