...
func (cs *ConsensusState) readReplayMessage(msg *consensus.TimedWALMessage, newStepSub ttypes.Subscription) error {
	// Skip meta messages which exist for demarcating boundaries.
	if _, ok := msg.Msg.(consensus.EndHeightMessage); ok {
		return nil
	}

	// for logging
	switch m := msg.Msg.(type) {
	case ttypes.EventDataRoundState:
		...
		if newStepSub != nil {
			...
			case stepMsg := <-newStepSub.Out():
				m2 := stepMsg.Data().(ttypes.EventDataRoundState)
...
func (cs *ConsensusState) catchupReplay(csHeight int64) error {
	...
	gr, found, err := cs.wal.SearchForEndHeight(csHeight, &consensus.WALSearchOptions{IgnoreDataCorruptionErrors: true})
	if err != nil {
		return err
	}
	if gr != nil {
	...
	gr, found, err = cs.wal.SearchForEndHeight(csHeight-1, &consensus.WALSearchOptions{IgnoreDataCorruptionErrors: true})
	if err == io.EOF {
...
func NewHandshaker(stateDB dbm.DB, state sm.State,
	store sm.BlockStore, genDoc *ttypes.GenesisDoc) *Handshaker {
...
// If not called, it defaults to ttypes.NopEventBus.
func (h *Handshaker) SetEventBus(eventBus ttypes.BlockEventPublisher) {
	h.eventBus = eventBus
}
...
func (h *Handshaker) ReplayBlocks(
	...
	if appBlockHeight == 0 {
		validators := make([]*ttypes.Validator, len(h.genDoc.Validators))
		for i, val := range h.genDoc.Validators {
			validators[i] = ttypes.NewValidator(val.PubKey, val.Power)
		}
		validatorSet := ttypes.NewValidatorSet(validators)
		nextVals := ttypes.TM2PB.ValidatorUpdates(validatorSet)
		csParams := ttypes.TM2PB.ConsensusParams(h.genDoc.ConsensusParams)
		...
		if stateBlockHeight == 0 { //we only update state when we are in initial state
			...
			if len(res.Validators) > 0 {
				vals, err := ttypes.PB2TM.ValidatorUpdates(res.Validators)
				if err != nil {
					return nil, err
				}
				state.Validators = ttypes.NewValidatorSet(vals)
				state.NextValidators = ttypes.NewValidatorSet(vals)
			} else {
	...
	if storeBlockHeight == 0 {
		...
		return appHash, tsm.ErrAppBlockHeightTooHigh{CoreHeight: storeBlockHeight, AppHeight: appBlockHeight}
	...
	if storeBlockHeight == stateBlockHeight {
		...
		if appBlockHeight < stateBlockHeight {
			...
			abciResponses, err := tsm.LoadABCIResponses(h.stateDB, storeBlockHeight)
			if err != nil {
				return nil, err
			}
...
func (h *Handshaker) replayBlock(state sm.State, height int64, proxyApp proxy.AppConnConsensus) (sm.State, error) {
	...
	blockExec := sm.NewBlockExecutor(h.stateDB, h.logger, proxyApp, mock.Mempool{}, mock.Mempool{}, sm.MockEvidencePool{})
...
func newMockProxyApp(appHash []byte, abciResponses *tsm.ABCIResponses) proxy.AppConnConsensus {
