...
func startNewConsensusStateAndWaitForBlock(t *testing.T, consensusReplayConfig *cfg.Config,
	...
	newBlockSub, err := cs.eventBus.Subscribe(context.Background(), testSubscriber, ttypes.EventQueryNewBlock)
	require.NoError(t, err)
	select {
...
var _ consensus.WAL = &crashingWAL{}
...
func (w *crashingWAL) Write(m consensus.WALMessage) {
	if endMsg, ok := m.(consensus.EndHeightMessage); ok {
...
func (w *crashingWAL) WriteSync(m consensus.WALMessage) {
	w.Write(m)
}

func (w *crashingWAL) FlushAndSync() error { return w.next.FlushAndSync() }

func (w *crashingWAL) SearchForEndHeight(height int64, options *consensus.WALSearchOptions) (rd io.ReadCloser, found bool, err error) {
...
func TestSimulateValidatorsChange(t *testing.T) {
	...
	partSize := ttypes.BlockPartSizeBytes

	newRoundCh := subscribe(css[0].eventBus, ttypes.EventQueryNewRound)
	proposalCh := subscribe(css[0].eventBus, ttypes.EventQueryCompleteProposal)
	...
	signAddVotes(css[0], ttypes.PrecommitType, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), vss[1:nVals]...)
	...
	valPubKey1ABCI := ttypes.TM2PB.PubKey(newValidatorPubKey1)
	newValidatorTx1 := kvstore.MakeValSetChangeTx(valPubKey1ABCI, testMinPower)
	err := assertMempool(css[0].txNotifier).CheckTx(newValidatorTx1, nil)
	assert.Nil(t, err)
	propBlock, _ := css[0].createProposalBlock() //changeProposer(t, cs1, vs2)
	propBlockParts := propBlock.MakePartSet(partSize)
	blockID := ttypes.BlockID{Hash: propBlock.Hash(), PartsHeader: propBlockParts.Header()}
	proposal := ttypes.NewProposal(vss[1].Height, round, -1, blockID)
	...
	updatePubKey1ABCI := ttypes.TM2PB.PubKey(updateValidatorPubKey1)
	...
	propBlockParts = propBlock.MakePartSet(partSize)
	blockID = ttypes.BlockID{Hash: propBlock.Hash(), PartsHeader: propBlockParts.Header()}
	proposal = ttypes.NewProposal(vss[2].Height, round, -1, blockID)
	...
	newVal2ABCI := ttypes.TM2PB.PubKey(newValidatorPubKey2)
	newValidatorTx2 := kvstore.MakeValSetChangeTx(newVal2ABCI, testMinPower)
	err = assertMempool(css[0].txNotifier).CheckTx(newValidatorTx2, nil)
	assert.Nil(t, err)
	newValidatorPubKey3 := css[nVals+2].privValidator.GetPubKey()
	newVal3ABCI := ttypes.TM2PB.PubKey(newValidatorPubKey3)
	...
	proposal = ttypes.NewProposal(vss[3].Height, round, -1, blockID)
	...
	for i := 0; i < nVals+1; i++ {
		...
		signAddVotes(css[0], ttypes.PrecommitType, rs.ProposalBlock.Hash(), rs.ProposalBlockParts.Header(), newVss[i])
	}
	...
	sim.Commits = make([]*ttypes.Commit, 0)
...
func TestMockProxyApp(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		abciResWithEmptyDeliverTx := new(tsm.ABCIResponses)
		...
		loadedAbciRes := new(tsm.ABCIResponses)
...
func testHandshakeReplay(t *testing.T, config *cfg.Config, nBlocks int, mode uint, testValidatorsChange bool) {
	...
	if testValidatorsChange {
		...
		defer os.RemoveAll(testConfig.RootDir)
		walBody, err := consensus.WALWithNBlocks(t, numBlocks)
		require.NoError(t, err)
...
func applyBlock(stateDB dbm.DB, st sm.State, blk *types.Block, proxyApp proxy.AppConns) sm.State {
	testPartSize := ttypes.BlockPartSizeBytes
	blockExec := sm.NewBlockExecutor(stateDB, log.TestingLogger(), proxyApp.Consensus(), mempool, commitpool, evpool)

	blkID := ttypes.BlockID{Hash: blk.Hash(), PartsHeader: blk.MakePartSet(testPartSize).Header()}
...
func buildAppStateFromChain(proxyApp proxy.AppConns, stateDB dbm.DB,
	...
	validators := ttypes.TM2PB.ValidatorUpdates(state.Validators)
...
func makeBlocks(n int, state *sm.State, privVal ttypes.PrivValidator) []*types.Block {
	blocks := make([]*types.Block, 0)

	var (
		prevBlock     *types.Block
		prevBlockMeta *ttypes.BlockMeta
	)
	...
	for i := 0; i < n; i++ {
		...
		prevBlock = block
		prevBlockMeta = ttypes.NewBlockMeta(block.GetTendermintBlock(), parts)
...
func makeVote(header *ttypes.Header, blockID ttypes.BlockID, valset *ttypes.ValidatorSet, privVal ttypes.PrivValidator) *ttypes.Vote {
...
func makeBlock(state sm.State, lastBlock *types.Block, lastBlockMeta *ttypes.BlockMeta,
	privVal ttypes.PrivValidator, height int64) (*types.Block, *ttypes.PartSet) {

	lastCommit := ttypes.NewCommit(ttypes.BlockID{}, nil)
	if height > 1 {
		vote := makeVote(&lastBlock.Header, lastBlockMeta.BlockID, state.Validators, privVal).CommitSig()
		lastCommit = ttypes.NewCommit(lastBlockMeta.BlockID, []*ttypes.CommitSig{vote})
	}

	return state.MakeBlock(height, []ttypes.Tx{}, nil, lastCommit, nil, state.Validators.GetProposer().Address)
}
...
func makeBlockchainFromWAL(wal consensus.WAL) ([]*types.Block, []*ttypes.Commit, error) {
	var height int64

	// Search for height marker
	gr, found, err := wal.SearchForEndHeight(height, &consensus.WALSearchOptions{})
	if err != nil {
		return nil, nil, err
	}
	if !found {
	...
	var (
		blocks          []*types.Block
		commits         []*ttypes.Commit
		thisBlockParts  *ttypes.PartSet
		thisBlockCommit *ttypes.Commit
	)
	...
	for {
		...
		case *ttypes.PartSetHeader:
			thisBlockParts = ttypes.NewPartSetFromHeader(*p)
		case *ttypes.Part:
		...
		case *ttypes.Vote:
			if p.Type == ttypes.PrecommitType {
				commitSigs := []*ttypes.CommitSig{p.CommitSig()}
				thisBlockCommit = ttypes.NewCommit(p.BlockID, commitSigs)
			}
		}
	}
...
func readPieceFromWAL(msg *consensus.TimedWALMessage) interface{} {
	// for logging
...
func newMockBlockStore(config *cfg.Config, params ttypes.ConsensusParams) *mockBlockStore {
	return &mockBlockStore{config, params, nil, nil}
}

func (bs *mockBlockStore) Height() int64                       { return int64(len(bs.chain)) }
func (bs *mockBlockStore) LoadBlock(height int64) *types.Block { return bs.chain[height-1] }
func (bs *mockBlockStore) LoadBlockMeta(height int64) *ttypes.BlockMeta {
	block := bs.chain[height-1]
	return &ttypes.BlockMeta{
		BlockID: ttypes.BlockID{Hash: block.Hash(), PartsHeader: block.MakePartSet(ttypes.BlockPartSizeBytes).Header()},
		Header:  block.Header,
	}
}
func (bs *mockBlockStore) LoadBlockPart(height int64, index int) *ttypes.Part { return nil }
func (bs *mockBlockStore) SaveBlock(block *types.Block, blockParts *ttypes.PartSet, seenCommit *ttypes.Commit) {
}
func (bs *mockBlockStore) LoadBlockCommit(height int64) *ttypes.Commit {
	return bs.commits[height-1]
}
func (bs *mockBlockStore) LoadSeenCommit(height int64) *ttypes.Commit {
	return bs.commits[height-1]
}
...
func TestHandshakeUpdatesValidators(t *testing.T) {
	val, _ := ttypes.RandValidator(true, 10)
	vals := ttypes.NewValidatorSet([]*ttypes.Validator{val})
	app := &initChainApp{vals: ttypes.TM2PB.ValidatorUpdates(vals)}
	clientCreator := proxy.NewLocalClientCreator(app)
