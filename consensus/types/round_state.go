...
type RoundState struct {
	...
	Proposal                  *ttypes.Proposal     `json:"proposal"`
	ProposalBlock             *types.Block         `json:"proposal_block"`
	ProposalBlockParts        *ttypes.PartSet      `json:"proposal_block_parts"`
	LockedRound               int                  `json:"locked_round"`
	LockedBlock               *types.Block         `json:"locked_block"`
	LockedBlockParts          *ttypes.PartSet      `json:"locked_block_parts"`
	ValidRound                int                  `json:"valid_round"`       // Last known round with POL for non-nil valid block.
	ValidBlock                *types.Block         `json:"valid_block"`       // Last known block of POL mentioned above.
	ValidBlockParts           *ttypes.PartSet      `json:"valid_block_parts"` // Last known block parts of POL metnioned above.
	Votes                     *HeightVoteSet       `json:"votes"`
	CommitRound               int                  `json:"commit_round"` //
	LastCommit                *ttypes.VoteSet      `json:"last_commit"`  // Last precommits at Height-1
	LastValidators            *ttypes.ValidatorSet `json:"last_validators"`
...
func (rs *RoundState) NewRoundEvent() ttypes.EventDataNewRound {
	addr := rs.Validators.GetProposer().Address
	idx, _ := rs.Validators.GetByAddress(addr)

	return ttypes.EventDataNewRound{
		Height: rs.Height,
		Round:  rs.Round,
		Step:   rs.Step.String(),
		Proposer: ttypes.ValidatorInfo{
			Address: addr,
			Index:   idx,
		},
	}
...
func (rs *RoundState) CompleteProposalEvent() ttypes.EventDataCompleteProposal {
	...
	return ttypes.EventDataCompleteProposal{
		Height:  rs.Height,
		Round:   rs.Round,
		Step:    rs.Step.String(),
		BlockID: blockId,
...
func (rs *RoundState) RoundStateEvent() ttypes.EventDataRoundState {
	return ttypes.EventDataRoundState{
		Height: rs.Height,
		Round:  rs.Round,
		Step:   rs.Step.String(),
	}
