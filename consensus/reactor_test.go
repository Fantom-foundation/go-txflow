...
func startConsensusNet(t *testing.T, css []*ConsensusState, N int) (
	...
	blocksSubs := make([]ttypes.Subscription, 0)
	eventBuses := make([]*ttypes.EventBus, N)
	for i := 0; i < N; i++ {
		...
		blocksSub, err := eventBuses[i].Subscribe(context.Background(), testSubscriber, ttypes.EventQueryNewBlock)
...
func stopConsensusNet(logger log.Logger, reactors []*ConsensusReactor, eventBuses []*ttypes.EventBus) {
...
func TestReactorWithEvidence(t *testing.T) {
	ttypes.RegisterMockEvidences(cdc)
	ttypes.RegisterMockEvidences(ttypes.GetCodec())
	...
	for i := 0; i < nValidators; i++ {
		...
		// Make Mempool
		commitpool := mempl.NewCListMempool(thisConfig.Mempool, nil, 0)
	...
	timeoutWaitGroup(t, nValidators, func(j int) {
		...
		block := msg.Data().(ttypes.EventDataNewBlock).Block
...
func newMockEvidencePool(val []byte) *mockEvidencePool {
	return &mockEvidencePool{
		ev: []ttypes.Evidence{ttypes.NewMockGoodEvidence(1, 1, val)},
	}
}

// NOTE: maxBytes is ignored
func (m *mockEvidencePool) PendingEvidence(maxBytes int64) []ttypes.Evidence {
	if m.height > 0 {
		return m.ev
	}
	return nil
...
func TestReactorReceiveDoesNotPanicIfAddPeerHasntBeenCalledYet(t *testing.T) {
	...
	var (
		...
		msg     = cdc.MustMarshalBinaryBare(&HasVoteMessage{Height: 1, Round: 1, Index: 1, Type: ttypes.PrevoteType})
	)
...
func TestReactorVotingPowerChange(t *testing.T) {
	...
	val1PubKeyABCI := ttypes.TM2PB.PubKey(val1PubKey)
...
func waitForAndValidateBlock(
	...
	blocksSubs []ttypes.Subscription,
	css []*ConsensusState,
	txs ...[]byte,
) {
	timeoutWaitGroup(t, n, func(j int) {
		...
		msg := <-blocksSubs[j].Out()
		newBlock := msg.Data().(ttypes.EventDataNewBlock).Block
...
func validateBlock(block *ttypes.Block, activeVals map[string]struct{}) error {
