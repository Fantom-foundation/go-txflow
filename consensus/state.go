...
func (cs *ConsensusState) OnStart() error {
	...
	if cs.doWALCatchup {
		if err := cs.catchupReplay(cs.Height); err != nil {
			...
			if consensus.IsDataCorruptionError(err) {
...
func (cs *ConsensusState) OpenWAL(walFile string) (consensus.WAL, error) {
...
func (cs *ConsensusState) AddVote(vote *ttypes.Vote, peerID p2p.ID) (added bool, err error) {
	if peerID == "" {
...
func (cs *ConsensusState) SetProposal(proposal *ttypes.Proposal, peerID p2p.ID) error {
...
func (cs *ConsensusState) AddProposalBlockPart(height int64, round int, part *ttypes.Part, peerID p2p.ID) error {
...
func (cs *ConsensusState) SetProposalAndBlock(proposal *ttypes.Proposal, block *types.Block, parts *ttypes.PartSet, peerID p2p.ID) error {
...
func (cs *ConsensusState) reconstructLastCommit(state sm.State) {
	...
	lastPrecommits := ttypes.CommitToVoteSet(state.ChainID, seenCommit, state.LastValidators)
...
func (cs *ConsensusState) defaultDecideProposal(height int64, round int) {
	...
	// Make proposal
	propBlockId := ttypes.BlockID{Hash: block.Hash(), PartsHeader: blockParts.Header()}
	proposal := ttypes.NewProposal(height, round, cs.ValidRound, propBlockId)
...
func (cs *ConsensusState) defaultDoPrevote(height int64, round int) {
	...
	if cs.LockedBlock != nil {
		...
		cs.signAddVote(ttypes.PrevoteType, cs.LockedBlock.Hash(), cs.LockedBlockParts.Header())
		return
	}
	...
	if cs.ProposalBlock == nil {
		...
		cs.signAddVote(ttypes.PrevoteType, nil, ttypes.PartSetHeader{})
		return
	}
	...
	cs.signAddVote(ttypes.PrevoteType, cs.ProposalBlock.Hash(), cs.ProposalBlockParts.Header())
}
...
func (cs *ConsensusState) enterPrecommit(height int64, round int) {
	...
	if !ok {
		...
		cs.signAddVote(ttypes.PrecommitType, nil, ttypes.PartSetHeader{})
		return
	}
	...
	if cs.LockedBlock.HashesTo(blockID.Hash) {
		...
		cs.signAddVote(ttypes.PrecommitType, blockID.Hash, blockID.PartsHeader)
		return
	}
...
func (cs *ConsensusState) enterCommit(height int64, commitRound int) {
	...
	if !cs.ProposalBlock.HashesTo(blockID.Hash) {
		if !cs.ProposalBlockParts.HasHeader(blockID.PartsHeader) {
			...
			cs.evsw.FireEvent(ttypes.EventValidBlock, &cs.RoundState)
		} else {
...
func (cs *ConsensusState) finalizeCommit(height int64) {
	...
	// restart).
	cs.wal.WriteSync(consensus.EndHeightMessage{height}) // NOTE: fsync
	...
	var err error
	stateCopy, err = cs.blockExec.ApplyBlock(stateCopy, ttypes.BlockID{Hash: block.Hash(), PartsHeader: blockParts.Header()}, block)
	if err != nil {
...
func (cs *ConsensusState) defaultSetProposal(proposal *ttypes.Proposal) error {
	...
	if cs.ProposalBlockParts == nil {
		cs.ProposalBlockParts = ttypes.NewPartSetFromHeader(proposal.BlockID.PartsHeader)
	}
...
func (cs *ConsensusState) tryAddVote(vote *ttypes.Vote, peerID p2p.ID) (bool, error) {
...
func (cs *ConsensusState) addVote(vote *ttypes.Vote, peerID p2p.ID) (added bool, err error) {
	...
	if vote.Height+1 == cs.Height {
		if !(cs.Step == cstypes.RoundStepNewHeight && vote.Type == ttypes.PrecommitType) {
			// TODO: give the reason ..
			// fmt.Errorf("tryAddVote: Wrong height, not a LastCommit straggler commit.")
			return added, consensus.ErrVoteHeightMismatch
		}
		...
		cs.eventBus.PublishEventVote(ttypes.EventDataVote{Vote: vote})
	...
	if vote.Height != cs.Height {
		err = consensus.ErrVoteHeightMismatch
...
func (cs *ConsensusState) signVote(type_ ttypes.SignedMsgType, hash []byte, header ttypes.PartSetHeader) (*ttypes.Vote, error) {
...
func (cs *ConsensusState) signAddVote(type_ ttypes.SignedMsgType, hash []byte, header ttypes.PartSetHeader) *ttypes.Vote {
