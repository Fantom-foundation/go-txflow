...
func makeAndConnectEventpoolReactors(cfg *cfg.Config, N int) []*EventpoolReactor {
	...
	for i := 0; i < N; i++ {
		...
		reactors[i] = NewEventpoolReactor(config.TestEventpoolConfig(), eventpool) // so we dont start the consensus states
